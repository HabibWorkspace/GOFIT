"""
Script to add members to production database
"""
import psycopg2
    ...
    try:
        ...
        for pkg_name in ["Basic", "Cardio", "Combo I", "Combo III", "Aerobic"]:
            ...
            if pkg_id:
                ...
                print(f"Found package: {pkg_name} -> {pkg_id}")
        
        # Get trainer IDs
        trainers = {}
        for trainer_name in ["Ali", "Mustafa"]:
            trainer_id = get_trainer_id(cursor, trainer_name)
            if trainer_id:
                trainers[trainer_name] = trainer_id
                print(f"Found trainer: {trainer_name} -> {trainer_id}")
        
        print(f"\nAdding {len(members_data)} members...")
        ...
        for member in members_data:
            try:
                ...
                if member["phone"]:
                    cursor.execute("SELECT id FROM member_profiles WHERE phone = %s", (member["phone"],))
                    if cursor.fetchone():
                        print(f"  ⊘ Skipped {member['name']} (phone already exists)")
                        skipped += 1
                        continue
                ...
                print(f"  ✓ Added {member['name']} - {member['package']}" + 
                      (f" (Trainer: {member['trainer']})" if member['trainer'] else ""))
                added += 1
        ...
        if skipped > 0:
            print(f"⊘ Skipped {skipped} members (already exist)")
